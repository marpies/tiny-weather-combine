...
class CoreDataService_LocationWeather_Tests: XCTestCase, CoreDataTestingSetup {
    ...
    func testSavingLocationWeather() throws {
        ...
        self.saveAndVerifyWeather(weather1, location: location)
        
        // Update weather
        let weather2 = TestWeather.updated
        self.saveAndVerifyWeather(weather2, location: location)
    }
    ...
    func testLoadingLocationWeather() throws {
        ...
        let expectInitialNilWeather = expectation(description: "expected initial weather to not exist")
        let expectLoadedWeather = expectation(description: "expected to load weather")
        
        self.sut.loadLocationWeather(latitude: location.lat, longitude: location.lon)
            .sink(receiveCompletion: { completion in
                ...
                if response == nil {
                    expectInitialNilWeather.fulfill()
    ...
    private func saveAndVerifyWeather(_ weather: Weather.Overview.Response, location: WeatherLocation) {
        let expect = expectation(description: #function)
        
        self.sut.saveLocationWeather(weather, location: location)
            .sink(receiveCompletion: { completion in
                if case .failure(let error) = completion {
                    XCTFail(error.localizedDescription)
                }
