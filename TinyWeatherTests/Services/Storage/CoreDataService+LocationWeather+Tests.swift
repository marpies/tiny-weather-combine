...
class CoreDataService_LocationWeather_Tests: XCTestCase, CoreDataTestingSetup {
    ...
    func testLoadingLocationWeather() throws {
        ...
        let first = try self.sut.loadLocationWeather(latitude: location.lat, longitude: location.lon).toBlocking(timeout: 1).first()
        ...
        let loadedWeather = try XCTUnwrap(try self.sut.loadLocationWeather(latitude: location.lat, longitude: location.lon).toBlocking(timeout: 1).first())
    ...
    private func saveAndVerifyWeather(_ weather: Weather.Overview.Response, location: WeatherLocation) {
        let save = self.sut.saveLocationWeather(weather, location: location).toBlocking(timeout: 1).materialize()
